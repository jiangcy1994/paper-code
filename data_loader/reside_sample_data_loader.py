...
class RESIDEOTSSampleDataLoader(DataLoader):
    ...
    def train_data(self, include_trans_atmos=False, include_A_b=False):
        ...
        if include_trans_atmos:
            ...
            trans_ds = self._get_trans_ds(basename_list)
            atmos_ds = self._get_atmos_ds(basename_list)
            zip_ds = tf.data.Dataset.zip(
                (
                    atmos_ds, 
                    trans_ds.map(lambda x: tf.subtract(tf.ones(self.img_shape + (3,)), x)))
            )
            b_ds = zip_ds.map(lambda x1, x2: tf.multiply(x1, x2))

            return haze_train_img_ds, clear_train_img_ds, trans_ds, b_ds
