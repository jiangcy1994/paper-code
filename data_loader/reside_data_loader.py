...
class RESIDEOTSDataLoader():
    def __init__(self, img_res=(256, 256), 
                 clear_path='D:/DataSet/RESIDE/OTS_ALPHA/clear/clear_images/', 
                 haze_path='D:/DataSet/RESIDE/OTS_ALPHA/haze/OTS/'):
...
class RESIDEOTSDiscriminatorDataLoader():
    def __init__(self, img_res=(256, 256),
                 clear_path='D:/DataSet/RESIDE/OTS_ALPHA/clear/clear_images/', 
                 haze_path='D:/DataSet/RESIDE/OTS_ALPHA/haze/OTS/'):
        self.dataset_name = self.__class__
    ...
    def load_batch(self, batch_size=2, is_testing=False):
        haze_base_names = [os.path.basename(path) for path in glob(self.haze_path + '*' + self.img_type)]
        ...
        for i in range(self.n_batches-1):
            ...
            imgs = np.concatenate([np.array(clear_imgs), 
                                   np.array(haze_imgs)])
            results = np.concatenate([np.ones((len(batch_haze), 16, 16, 1)),
                                      np.zeros((len(batch_haze), 16, 16, 1))])
